...
const getAll = async () => {
    try {
        let res = await axios.get("http://localhost:4444/info")
    ...
    if (e.target === $form) {
        ...
        if (!e.target.id.value) {
            try {
                ...
                let res = await axios("http://localhost:4444/info", options)
                let json = await res.data;
            ...
            try {
                ...
                let res = await axios(`http://localhost:4444/info/${e.target.id.value}`, options)
                let json = await res.data;
    ...
    if (e.target.matches(".delete")) {
        ...
        if (confirmacion) {
            try {
                ...
                let res = await axios(`http://localhost:4444/info/${e.target.dataset.id}`, options)
                let json = await res.data;
